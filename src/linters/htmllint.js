...
class Htmllint {
	...
	static async verifySetup(dir, prefix = "") {
		// Verify that NPM is installed (required to execute Htmllint)
		if (!(await commandExists("npm"))) {
			throw new Error("NPM is not installed");
		}

		// Verify that Htmllint is installed
		const commandPrefix = prefix || getNpmBinCommand(dir);
		try {
			run(`${commandPrefix} htmllint -v`, { dir });
		} catch (err) {
	...
	static lint(dir, extensions, args = "", fix = false, prefix = "") {
		...
		const commandPrefix = prefix || getNpmBinCommand(dir);
		return run(`${commandPrefix} htmllint ${fixArg} --no-color ${args} "${files}"`, {
			dir,
			ignoreErrors: true,
		});
	}
	...
	static parseOutput(dir, output) {
		...
		lintResult.error = paths.map((path) => ({
			...
			lastLine: 1,
			message:
				"There are issues with this file's formatting, please run Htmllint to fix the errors",
		}));
